...
yvars <- list('Anxiety' = c(var_name='Q31A_anxiety_dich', data_prefix='dx_anxiety_all_covariate'),
              ...
              'Panic Attacks' = c(var_name='Q31B_panic_dich', data_prefix='dx_panic_all_covariate'), 
              'Schizophrenia' = c(var_name='Q31B_schizo_dich', data_prefix='dx_schizo_all_covariate'), 
              'Suicidal Thoughts' = c(var_name='Q30J_suic_thnk_r_12mos', data_prefix='suic_thnk_all_covariate'), 
              'Suicide Attempt' = c(var_name='Q30K_suic_try_r_12mos', data_prefix='suic_try_all_covariate'),
              'Poor Health' = c(var_name='global_health_dich', data_prefix='global_health_all_covariate'), 
              'Emotional Distress' = c(var_name='neg_emo_any', data_prefix='neg_emo_any_all_cov'),
              'Overwhelmed' = c(var_name='Q30B_overwhelmed_r_2wks', data_prefix='ovrwhlm_item_all_cov'),
              'Exhausted' = c(var_name='Q30C_exhausted_r_2wks', data_prefix='exhausted_item_all_cov'),
              'Overwhelmed or Exhausted' = c(var_name='ovrwhlm_any', data_prefix='ovrwhlm_any_all_cov'), 
              'Sad'= c(var_name="Q30E_sad_r_2wks", data_prefix='sad_all_covariate'),
              'Anger'= c(var_name="Q30H_anger_r_2wks", data_prefix='anger_all_covariate'),
...
for(yvar in names(yvars)){
    ...
    if(file.exists(data_filepath)) {
        ...
        load_data_path <- '{SUMMARY_OUTPUT}/ACHA-NCHA_{yvar}_summary_data_all_cov.RData' %>% glue()
        ...
        if(file.exists(load_data_path)) {
            ...
            lv2_post <- posterior_samples(model, pars = c('b_Intercept', 'b_c_Time', 'b_quad_c_Time'))
        ...
        save(list = c('data', 'model', 'plot_df', 'summary_plot'), 
             file = '{SUMMARY_OUTPUT}/ACHA-NCHA_{yvar}_summary_data_all_cov.RData' %>% glue())
