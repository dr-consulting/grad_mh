...
yvars <- list('Suicide Attempt' = c(var_name='Q30K_suic_try_r_12mos', data_prefix='dx_suic_try_no_cov_linear'),
              ...
              'Overwhelmed or Exhausted' = c(var_name='ovrwhlm_any', data_prefix='ovrwhlm_any_no_cov_linear'))
...
for(yvar in names(yvars)){
    ...
    if(file.exists(data_filepath)) {
        ...
        load_data_path <- '{SUMMARY_OUTPUT}/ACHA-NCHA_{yvar}_no_cov_linear_summary_data.RData' %>% glue()
        ...
        save(list = c('data', 'model', 'plot_df', 'summary_plot'), 
             file = '{SUMMARY_OUTPUT}/ACHA-NCHA_{yvar}_no_cov_linear_summary_data.RData' %>% glue())
