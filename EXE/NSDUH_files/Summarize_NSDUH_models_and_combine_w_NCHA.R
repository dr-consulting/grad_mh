...
plot_palette <- c('black', analogous_palette[1:2])
names(plot_palette) <- c('Fitted All Adults', 'ACHA-NCHA', 'Fitted Matched Adults')
...
for(yvar in names(yvars)){
    ...
    if(file.exists(data_filepath)) {
        ...
        if(grepl('Matched', yvar)) {
            plt <- plt + 
                geom_ribbon(aes(ymax=.upper * 100, ymin=.lower * 100, 
...
for(var in base_vars) {
    if(var %in% names(ncha_yvars)){
        ...
        tmp_plot <- plotlist[[max(list_pos)]] +
            ...
            geom_ribbon(data = acha_plot_df, aes(x = c_Time, ymin = lb, ymax = ub, fill='ACHA-NCHA'), alpha=.5) +
