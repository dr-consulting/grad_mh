...
# Will create weights using the inverse of the probability of being in the ACHA-II sample each year
...
weighted_df <- grads_only_study_df %>%
    ...
    select(sex, age_buckets, new_weights, prop_weights)

nsduh_matched_df <- merge(nsduh_matched_df, weighted_df, by = c('sex', 'age_buckets'))

nsduh_weighted_summary_df <- nsduh_matched_df %>% 
    group_by(c_Time) %>% 
    summarize(
        anymi_12mos = sum(anymi_12mos * new_weights) / sum(new_weights) * 100,
        suic_thnk_12mos= sum(as.numeric(suic_thnk_12mos == 'Yes') * new_weights, na.rm = TRUE) / sum(new_weights) * 100, 
        suic_try_12mos = sum(as.numeric(suic_try_12mos == 'Yes') * new_weights, na.rm = TRUE) / sum(new_weights) * 100,
        global_health_dich = sum(as.numeric(global_health %in% c('Poor', 'Fair')) * new_weights) / sum(new_weights) *100,
    ) %>% 
    ungroup() %>% 
