...
"
res_1 <- find_single_factor_final_model(one_fact_base, data = grad_train, ordered_vars = vars, p_thresh = 1e-5)
...
F2 =~ Q30B_overwhelmed_r + Q30C_exhausted_r
"

valid_covars <- names(label_map)[label_map %in% emo_distress]

# Had to run through the comparisons and then add in iteratively error covariances that included heywood cases
invalid_covars <- c(
  "Q30D_lonely_r~~Q30G_anxiety_r"
)

res_2 <- find_dual_factor_final_model(two_fact_base, data = grad_train, ordered_vars = vars, valid_covars = valid_covars, 
...
```{r}
three_fact_base <- "
F1 =~ Q30A_hopeless_r + Q30D_lonely_r + Q30E_sad_r + Q30F_depressed_r 
F2 =~ Q30B_overwhelmed_r + Q30C_exhausted_r
F3 =~ Q30G_anxiety_r + Q30H_anger_r 
"
res_3 <- find_dual_factor_final_model(three_fact_base, data = grad_train, ordered_vars = vars, valid_covars = valid_covars[1:4], 
...
grad_comb <- rbind(grad_train[,c(vars, "train_flag")], grad_test[,c(vars, "train_flag")])
```
...
fit_one_test_fixed <- cfa(one_fact_test_fixed, data=grad_test, ordered=vars)
...
fit_two_test_fixed <- cfa(two_fact_test_fixed, data=grad_test, ordered=vars)
...
fit_three_test_fixed <- cfa(three_fact_test_fixed, data=grad_test, ordered=vars)
...
fit_hier_base <- cfa(hier_fact_base, data = grad_comb, ordered = vars, std.lv=TRUE)
lavaanPlot::lavaanPlot(model = fit_hier_base, labels = label_map, graph_options = list(fontsize = 9))
```
